...
- Generated audio or video object keys.
...
Static manual preview pages are served by Spring MVC from `backend/src/main/resources/static`: `/veo-video.html` for the new direct video flow and `/talking-head.html` for the retained audio-backed TalkingHead option.

## AI Providers

OpenAI is used for word information generation through Spring AI.

Google Veo is used for default direct pronunciation video generation. The configurable default model is `veo-3.1-fast-generate-preview`, which is the Veo 3.1 Lite/Fast path. Veo requests default to `9:16` vertical output.
...
OpenAI is also used for optional pronunciation audio generation through Spring AI `TextToSpeechModel` when requests use `renderMode: "talking-head"`.
